...
public class UserMapper {
	...
	public static Optional<UserDTO> UserToDTO(User user)
	{
		...
		return Optional.of(UserDTO.Builder.newInstance(Long.toString(user.getId()),
				user.getUsername(), user.getPassword(), user.getRole()).build());
		
	}
	public static Optional<User> DTOToUser(UserDTO user)
	{
		LOG.debug("Starting converting UserDTO id " + user.getId() +" to user ");
		return Optional.of(User.Builder.newInstance(Long.parseLong(user.getId()),
				user.getUsername(), user.getPassword(), user.getRole()).build());
	}
}
