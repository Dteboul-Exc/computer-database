...
/**
 * @author dteboul Backend validator for Servlet and ServiceComputer, throwing a
 *         custom esception in case of an unexpected value
 */
@Component
public class ComputerValidator {
	...
	/**
	 * Checks if introduced isn't set while discontinued is set and if introduced is
	 * set after discontinued
	 * 
	 * @param introduced
	 * @param discontinued
	 * @return
	...
	/**
	 *
	 * Various checks to very if the format of both dates are correct if they are
	 * set
	 * 
	 * @param introduced
	 * @param discontinued
	...
	public static boolean isDateValid(String introduced, String discontinued) throws ServiceComputerException {
		...
		if ((introduced.isEmpty()) && (discontinued.isEmpty()))
			return true;
		if (((introduced.length() != 10) && (introduced.length() != 0)))
			throw new ServiceComputerException("invalid Date , size  of introduced  is not valid");
		if (introduced.contains("/"))
			throw new ServiceComputerException("invalid introduced Date format, must not use / in the input string");
		if ((Integer.parseInt(introduced.substring(5, 7)) > 12) && (introduced.length() != 0))
			throw new ServiceComputerException("Date Mismatch. Month is placed in place of the day");
		if (Integer.parseInt(introduced.substring(8)) > 31)
			throw new ServiceComputerException("Date Mismatch. day is over 31");
		try {
			if (introduced.length() != 0)
				DateMapper.StringConverterInput(introduced);

			if (discontinued.length() != 0) {
				if (discontinued.contains("/"))
					throw new ServiceComputerException(
							"invalid discontinued Date format, must not use / in the input string");
				if (discontinued.length() != 10)
					throw new ServiceComputerException("invalid Date , size of discontinued  is not valid");
				if (Integer.parseInt(discontinued.substring(5, 7)) > 12)
					throw new ServiceComputerException("Date Mismatch. Month is placed in place of the day");
				if (Integer.parseInt(discontinued.substring(8)) > 31)
					throw new ServiceComputerException("Date Mismatch. day is over 31");
